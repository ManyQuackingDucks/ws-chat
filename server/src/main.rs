...
async fn main() {
    ...
    loop {
        ...
        match wshandler::accept_async(client.0).await {
            Ok(wsstream) => conn_threads.push(tokio::spawn(client_init(wsstream, tx.clone(), db_pool.get().unwrap()))),
            Err(e) => eprintln!("Connection error with client at {} [{e}]", client.1),
        }
    }
}
async fn client_init(conn: wshandler::WebSocketStream<TcpStream>, txch: broadcast::Sender<String>, db_conn: db::DbConnType) -> anyhow::Result<()>{
...
async fn client_recv(mut rx: types::SplitStream, txch: broadcast::Sender<String>, userdata: &types::LoggedInUser) -> anyhow::Result<()>{
