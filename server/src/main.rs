        ...
        if let Ok(client) = listener.accept().await {
            match wshandler::accept_async(client.0).await {
                Ok(wsstream) => conn_threads.push(tokio::spawn(client_init(
                    wsstream,
                    tx.clone(),
                    db_pool.get().unwrap(), //If db connections can not be spawned then the server is should exit
                ))),
...
async fn client_send(mut conn: types::SplitSink, mut rx: broadcast::Receiver<String>) {
    loop {
        ...
        conn.send(Message::Text(latest_mes)).await.unwrap(); //Shut down client thread if can not send to client
    }
}
